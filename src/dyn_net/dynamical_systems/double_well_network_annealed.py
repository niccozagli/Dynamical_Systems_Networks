...
class DoubleWellNetworkAnnealedParams(BaseModel):
...
def F(x, t, p: DoubleWellNetworkAnnealedParams) -> np.ndarray:
    n = x.size // 2
    X = x.reshape(n, 2)
    x1 = X[:, 0]
    x2 = X[:, 1]
    ...
    mean_x1 = float(np.dot(deg, x1) / deg_sum)
    mean_x2 = float(np.dot(deg, x2) / deg_sum)

    coupling1 = -p.theta * deg * (x1 - mean_x1)
    coupling2 = -p.theta * deg * (x2 - mean_x2)
